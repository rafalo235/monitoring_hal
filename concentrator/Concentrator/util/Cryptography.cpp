...
namespace NUtil {
  ...
  {
    uint16_t crc;
    int bufferLength = dataSize;
    int pointer = dataSize * 8;
    char divisor[3] = { 0x8E, 0xEF, 0x80 }; //TODO
    char *copy = (char*) calloc(dataSize + sizeof(uint16_t) + 1, 1);
    char tmp;
    ...
    do {
        for (int i = 0 ; i < 8 ; i++ ) {
            ...
            shift(copy, bufferLength); // przesuń bity danych o 1
            pointer--;
        }
        bufferLength--;
    } while (pointer > 0);
